...
const PurchaseCompleteModal: React.FC<CompleteModalProps> = ({ po, onClose, onUpdated }) => {
        ...
        <header className="mb-3 flex items-start justify-between gap-2">
          <div>
            <h2 className="text-sm font-semibold text-primaryText">
              Purchase Completion – {po.poNumber ?? 'PO'}
            </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="space-y-1">
            ...
            <textarea
              ...
              placeholder="Optional: Add packing details, vendor info, etc."
            />
          </div>
...
const SalesPurchaseQueueView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        // POs currently sitting with Purchase
  ...
  const canSee =
    role === 'admin' || department === 'purchase' || authUser.department === 'purchase';
  if (!canSee) {
    return (
      <div className="rounded-xl border border-stroke bg-foreground p-6 text-sm text-primaryText">
        You do not have permission to access the Purchase Queue.
      </div>
    );
  }
      ...
      <div className="flex h-full flex-col gap-3">
        ...
        {data.length === 0 ? (
          <div className="rounded-xl border border-stroke bg-foreground p-6 text-center text-sm text-primaryText/80">
            No POs currently assigned to Purchase.
          </div>
        ) : (
